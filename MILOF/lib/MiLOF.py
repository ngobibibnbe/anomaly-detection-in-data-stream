...
def MILOF(configFile):
	...
	original_datastream=datastream = np.unique(datastream, axis=0)
	...
	while not exit:
		...
		if not exit:
			...
			#original_datastream = np.delete(original_datastream, range(0,hbuck), axis=0)
			...
			# print("Points.knn = ", Points.knn)
	#print("Scores =", Scores)
	return Scores,original_datastream